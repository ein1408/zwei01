...
use app\admin\service\Confluence;//前段方法
...
class User extends Common
{
    ...
    public function memberList()
    {
        if ($this->request->isAjax()) {
            ...
            $map = [];
            empty ($data['key']) || $map[] = ['account', 'like', '%' . $data['key'] . '%'];
            $list = Db::name('member')
                ->alias('m')
                ->join('member_level l', 'm.level_id = l.id')
                ->where($map)
                ->field('m.*,l.title')
                ->paginate($data['limit'], false, ['query' => $data]);
            $user_date = [];
            foreach ($list as $key => $val) {
                $val['parent_name'] = Db::name('member')->where('id = '.$val['parent_id'])->value('nickname');
    ...
    public function memberEdit()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $uid = $this->request->get('uid', 0, 'intval');
            if ($uid) {
                $list = Db::name('member')->where('id', '=', $uid)->hidden(['password'])->find();
                $this->assign('list', $list);
            }
        }
        $menu = Db::name('member_level')->order('id desc')->column('id,title');
        $menu[0] = '顶级菜单';
        ksort($menu);
    ...
    public function memberLevel()
    {
        if ($this->request->isAjax()) {
            ...
            empty ($key) || $map[] = ['title', 'like', '%' . $key . '%'];
            $list = Db::name('member_level')->where($map)->paginate($limit, false, ['query' => ['key' => $key], 'limit' => $limit]);
            $data = $list->items();
            foreach ($data as $key => $value) {
                $data[$key]['status'] = $value['status'] == 0?"不禁用":"禁用";
            }
            return show($data, 0, '', ['count' => $list->total()]);
        }
        return $this->fetch();
    }
    public function levelEdit(){
       if ($this->request->isPost()) {
            ...
            $uid = $this->request->get('uid', 0, 'intval');
            if ($uid) {
                $list = Db::name('member_level')->where('id', '=', $uid)->hidden(['password'])->find();
                $this->assign('list', $list);
            }
            return $this->fetch();
        }
    }
    public function memberParent()
    {
        if ($this->request->isAjax()) {
            $data = [
                'key' => $this->request->get('key', '', 'trim'),
                'parent_id' => $this->request->get('uid', '', 'trim'),
                'limit' => $this->request->get('limit', 10, 'intval'),
            ];
            $map = [];
            empty ($data['key']) || $map[] = ['account', 'like', '%' . $data['key'] . '%'];
    ...
    public function memberProhibit(){

        if($this->request->isPost()){
            $data = $this->request->post();
            $map = array();
            $map['is_prohibit'] = $data['status'];
            $res = Db::name('member')->where('id',$data['id'])->update($map);
        }
        if ($res) {
            $code = 1;
            $msg = '保存成功';
