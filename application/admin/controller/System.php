...
class System extends Common
{
    ...
    {
        if (!$this->request->isPost()) {
            $data = Config::where('name', 'commission_config')->find();
            $this->assign('data', $data);
            return $this->fetch();
        } else {
            $save = [
                'value' => [
                    'commission_rate' => $this->request->post('commission_rate', '', 'trim'),
                    'dealer_balance' => $this->request->post('dealer_balance', '', 'trim'),
                    'settlement_fee' => $this->request->post('settlement_fee', '', 'trim'),
                    'banker_profit' => $this->request->post('banker_profit', '', 'trim'),
                    'bottom_pour' => $this->request->post('bottom_pour', '', 'trim'),
                    'withdraw_commission' => $this->request->post('withdraw_commission', '', 'trim'),
                    'QQ' => $this->request->post('QQ', '', 'trim'),
                    'wechat' => $this->request->post('wechat', '', 'trim')
                ],
            ];
            $res = Config::update($save, ['name' => 'commission_config']);
            if ($res) {
                cache('commission_config', null);
                $this->success('修改成功', url('/admin/commission_config'));
            } else {
                $this->error('修改失败');
            }
        }
